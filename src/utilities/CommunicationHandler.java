...
public class CommunicationHandler {
    /**
     * This class facilitates the communication between client and server. It doesnt
     * impliment any algorithms but instead it takes care of low level data
     * communication.
     */
    ...
    public void start(String inIP, String inPort) {
         ...
         * does the handshake and helps us with low level communication between ds-sim
         * and our client
         */
    ...
    public void quit(String quitMsg, boolean connectionProblemOccured) {
        ...
        try {
            if (!connectionProblemOccured) {
                // if connection problem exists, there is no point trying to send those
                // messages.
                send("QUIT");
                recieve("QUIT");
            }
    ...
    public LinkedList<Server> getServersFromGETS(String command, boolean noSchedulingHasBeenDoneYet) {
        /**
         * This method is designed to accomodate methods like GETS {all, capable, avail}
         * where the first response is the number of things that will later be sent and
         * the second thing are those servers. The purpose of this method is to allow
         * for the code to be more decoupled and cleaner. This method first asks for the
         * number of records that will be sent back as a result of the input command and
         * then it goes through those lines of response and parses them. Due to how our
         * assignmed is structured we are confident that there will never be a job which
         * can not be done by at least 1 server
         */
        String msg;
        ...
        if (numServersInt != 0) {
            for (int i = 0; i < numServersInt; i++) {
                msg = recieve(null);
                Server server = Server.parseServerFromGETS(msg, noSchedulingHasBeenDoneYet);
                list.add(server);
            }
            send("OK");
        }
    ...
    public int getEstimatedTimeLengthOfQuedJobs(Server server) {
        ...
        send("EJWT " + server.getName() + " " + server.getIdAmongName());
